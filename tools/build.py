#!/usr/bin/env python3
"""
Сборка галереи: берёт отобранные исходники, срезает угловой водяной знак
редакции, делает веб-версии и вклеивает разметку в index.html.
...
# водяной знак редакции — синяя плашка в нижнем углу кадра. Допуск
# широкий: у съёмок разных лет знак печатается чуть разным оттенком
# (встречались и (1,133,198), и (0,128,199)).
WM_RGB = (1, 133, 198)
WM_TOL = (26, 16, 20)


def strip_watermark(im):
    """Ищет знак в нижних углах и подрезает кадр выше него."""
    W, H = im.size
...
def main(sel_path, base=""):
    ...
    print("\nкадров: %d, водяной знак срезан на %d" % (len(sel), cropped))
    for key, label in GENRES:
        if counts.get(key):
